...
module cognitiveServiceWrapper './modules/deployAiFoundry.bicep' = {
  params:{
    name: name
    kind: kind
    cMKKeyVault: cMKKeyVault
...
module cognitiveService_privateEndpoints 'br/public:avm/res/network/private-endpoint:0.11.0' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): {
    ...
    params: {
      name: privateEndpoint.?name ?? 'pep-${last(split(cognitiveServiceWrapper.outputs.resourceId, '/'))}-${privateEndpoint.?service ?? 'account'}-${index}'
      privateLinkServiceConnections: privateEndpoint.?isManualConnection != true
        ? [
            {
              name: privateEndpoint.?privateLinkServiceConnectionName ?? '${last(split(cognitiveServiceWrapper.outputs.resourceId, '/'))}-${privateEndpoint.?service ?? 'account'}-${index}'
              properties: {
                privateLinkServiceId: cognitiveServiceWrapper.outputs.resourceId
                groupIds: [
...
output resourceId string = cognitiveServiceWrapper.outputs.resourceId//cognitiveService.id

@description('The resource group the cognitive services account was deployed into.')
output resourceGroupName string = resourceGroup().name

@description('The service endpoint of the cognitive services account.')
output endpoint string = cognitiveServiceWrapper.outputs.endpoint//cognitiveService.properties.endpoint

@description('All endpoints available for the cognitive services account, types depends on the cognitive service kind.')
output endpoints endpointType = cognitiveServiceWrapper.outputs.endpoints//cognitiveService.properties.endpoints

@description('The principal ID of the system assigned identity.')
output systemAssignedMIPrincipalId string? = cognitiveServiceWrapper.outputs.?systemAssignedMIPrincipalId//cognitiveService.?identity.?principalId
